...
1. [ ] All specified Tower configuration fields should be shown and configurable as documented.
1. [ ] User defined by TACACS+ server should be able to log in Tower.
1. [ ] User not defined by TACACS+ server should not be able to log in Tower via TACACS+.
1. [ ] A user existing in TACACS+ server but not in Tower should be created after the first success log in.
1. [ ] TACACS+ backend should stop authentication attempt after configured timeout and should not block the authentication pipeline in any case.
1. [ ] If exceptions occur on TACACS+ server side, the exception details should be logged in Tower, and Tower should not authenticate that user via TACACS+.
