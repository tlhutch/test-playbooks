...
def terminate_virtual_machine(module, azure):
    ...
    # ADDED BY CHRIS MEYERS
    '''
    Always try and clean up unattached disks. Note this isn't logic that everyone would want upon
    calling terminate for a particular vm.

    Piggybacking off the existing delete_empty_services module parameter
    '''
    if delete_empty_services:
        disks = azure.list_disks()
        for disk in disks:
            if disk.attached_to is None:
                #print json.dumps({ 'msg' : 'deleting unattached disk %s' % disk.name })
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            delete_empty_services=dict(type='bool', default=False),
        )
    )
