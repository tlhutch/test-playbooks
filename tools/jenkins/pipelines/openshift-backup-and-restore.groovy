    ...
    post {
        always {
            ...
            node('jenkins-jnlp-agent') {
                script {
                    json = "{\"tower\":\"${params.TOWER_VERSION}\", \"url\": \"${env.RUN_DISPLAY_URL}\", \"component\":\"backup_restore\", \"status\":\"${currentBuild.result}\", \"tls\":\"${params.AWX_USE_TLS}\", \"fips\":\"false\", \"deploy\":\"cluster\", \"platform\":\"OpenShift\", \"bundle\":\"no\", \"ansible\":\"${params.ANSIBLE_VERSION}\"}"
                }
