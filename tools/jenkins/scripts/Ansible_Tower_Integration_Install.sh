...
ansible tower -i playbooks/inventory.log -m shell -a "ansible --version"  -e @playbooks/vars.yml -e ansible_user="{{ ec2_images[0].user }}" | tee ansible.version.output || true
# Also archive results of rpm -qa | grep ansible to get names of all ansible and ansible tower packages installed
ansible tower -i playbooks/inventory.log -m shell -a "rpm -qa | grep ansible"  -e @playbooks/vars.yml -e ansible_user="{{ ec2_images[0].user }}" | tee ansible.rpm.version || true
set -e
