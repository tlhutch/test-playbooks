...
class TestSmartInventory(Base_Api_Test):
    ...
    def test_host_sourced(self, factories):
        ...
        inv = factories.v2_inventory(organization=host.ds.inventory.ds.organization, kind='smart',
                                     host_filter='ansible_facts__ansible_distribution={0} and ansible_facts__ansible_distribution_version="{1}"'.format(facts.ansible_distribution, facts.ansible_distribution_version))

        hosts = inv.related.hosts.get()
        assert hosts.count == 1
        assert hosts.results.pop().id == host.id

    def test_host_update2(self, factories):
        host = factories.v2_host()
