...
class TestLDAP(APITest):
    ...
    def test_ldap_is_superuser_supercedes_is_system_auditor(self, v2, update_setting_pg, clean_user_orgs_and_teams, resource_with_schedule):
        ldap_settings = deepcopy(self.base_ldap_settings)
        ldap_settings['AUTH_LDAP_USER_FLAGS_BY_GROUP'] = {'is_superuser': ['cn=planetexpress_admins,cn=groups,cn=accounts,dc=testing,dc=ansible,dc=com'],
                                                          'is_system_auditor': ['cn=planetexpress_admins,cn=groups,cn=accounts,dc=testing,dc=ansible,dc=com']}
        update_setting_pg(v2.settings.get().get_endpoint(
            'ldap'), ldap_settings)
        with self.current_user('hfarnsworth', self.ldap_password):
            ...
            schedule.patch()
            schedule.delete()
        clean_user_orgs_and_teams(v2.users.get(username='hfarnsworth').results.pop())
