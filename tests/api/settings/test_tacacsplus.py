...
class TestTACACSPlus(APITest):
    ...
    def test_login_as_new_user(self, request, protocol, enable_tacacs_auth, v2, api_me_pg):
        enable_tacacs_auth(protocol)
        tacacs_config = config.credentials.tacacs_plus
        assert not v2.users.get(username=tacacs_config.user).count
        ...
        users = v2.users.get(username=tacacs_config.user).results
        assert len(users) == 1, \
            "User {0} should be created after authenticating with TACACS+ server".format(tacacs_config.user)

    @pytest.mark.parametrize('protocol', ['ascii', 'pap'])
    def test_repeated_login_with_tacacs_auth(self, request, protocol, enable_tacacs_auth, v2):
        enable_tacacs_auth(protocol)
        tacacs_config = config.credentials.tacacs_plus
        ...
        with self.current_user(username, password):
            assert user_id == v2.me.get().results.pop().id, "Found different user_id on second login"
    ...
    @pytest.mark.parametrize('protocol', ['ascii', 'pap'])
    def test_login_as_fake_user(self, protocol, enable_tacacs_auth, v2, api_me_pg):
        enable_tacacs_auth(protocol)
    ...
    @pytest.mark.parametrize('protocol', ['ascii', 'pap'])
    def test_tacacs_account_should_not_grant_access_to_existing_account(self, factories, v2, protocol,
                                                                        enable_tacacs_auth):
        enable_tacacs_auth(protocol)
        tacacs_config = config.credentials.tacacs_plus
        username, password = (tacacs_config.user, getattr(tacacs_config, protocol + '_pass'))
        assert not v2.users.get(username=username).count
    ...
    @pytest.mark.parametrize('protocol', ['ascii', 'pap'])
    def test_cannot_change_password_for_tacacs_user_in_tower(self, request, v2, enable_tacacs_auth, protocol):
        enable_tacacs_auth(protocol)
        tacacs_config = config.credentials.tacacs_plus
        username, password = (tacacs_config.user, getattr(tacacs_config, protocol + '_pass'))
        assert not v2.users.get(username=username).count
