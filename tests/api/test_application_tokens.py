...
class TestDeprecatedAuthTokenCompatability(TestTokenAuthenticationBase):
    ...
    def test_authtoken_successful_creation(self, version, content_type):
        ...
        token = json['token']
        assert token
        assert json['refresh_token'] is None
        assert json['scope'] == 'write'
    ...
    def test_authtoken_with_incorrect_json_credentials(self, version):
        ...
        resp = conn.post(
            ...
            json={'username': 'doesnotexist', 'password': 'xyz'}
        )
        assert resp.status_code == 401

    @pytest.mark.parametrize('version', ('v1', 'v2'))
    def test_authtoken_with_incorrect_formencoded_credentials(self, version):
        conn = Connection(qe_config.base_url)
        resp = conn.post(
            '/api/{}/authtoken/'.format(version),
            headers={'Content-Type': 'application/x-www-form-urlencoded'},
            data={'username': 'doesnotexist', 'password': 'xyz'}
        )
        assert resp.status_code == 401

    @pytest.mark.parametrize('version', ('v1', 'v2'))
    def test_authtoken_http_get(self, version):
        conn = Connection(qe_config.base_url)
        resp = conn.get(
            '/api/{}/authtoken/'.format(version),
        )
