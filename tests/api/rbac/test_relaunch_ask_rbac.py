...
class TestRelaunchAskRBAC(APITest):
    ...
    @pytest.mark.parametrize("patch_payload, launch_payload, deny", [
        ...
        (dict(ask_diff_mode_on_launch=True, diff_mode=True), dict(diff_mode=False), True),
        (dict(ask_verbosity_on_launch=True, verbosity=1), dict(verbosity=0), True),
        ...
        (dict(ask_job_type_on_launch=True, job_type='check'), dict(job_type='run'), True),
