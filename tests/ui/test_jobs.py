...
def test_jobs_no_data(api_job_templates_pg, api_projects_pg, ui_jobs):
    """Verify jobs page region visibility and behavior when no jobs or
    associated schedules are loaded into the system
    """
    for result in api_projects_pg.get().results:
        result.silent_delete()

    for result in api_job_templates_pg.get().results:
        result.silent_delete()

    ui_jobs.refresh()

    assert ui_jobs.jobs_tab.is_displayed(), (
        'jobs tab unexpectedly not displayed')
    ...
    assert ui_jobs.schedules_tab.is_displayed(), (
        'schedules tab unexpectedly not displayed')

    assert ui_jobs.schedules.is_clickable(), (
...
def test_jobs(job_template_with_schedule, ui_jobs):
    ...
    assert ui_jobs.jobs_tab.is_displayed(), (
        'jobs tab unexpectedly not displayed with data available')
    ...
    assert ui_jobs.schedules_tab.is_displayed(), (
        'schedules tab unexpectedly not displayed with data available')
